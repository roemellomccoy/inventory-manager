...
function Helmets(){
  ...
  function setArmor(e){
    localStorage.setItem('helmetId', e.target.id)
  }
    ...
    <div className='row' style={{marginBottom: '30px'}}>
      ...
      {helmets.map(helmet =>
        <div className='row'>
          ...
          <div className='item'>{helmet.weight}</div>
          <div className='item'>{helmet.armor}</div>
          <div className='item'><button id={helmet.id} onClick={setArmor}>Add to Character</button></div>
          <div className='item'><button id={helmet.id} onClick={deleteArmor}>Delete</button></div>
        </div>
      )}
      <form onSubmit={addNewArmor} className='form'>
        <label for='addHelmet'>Armor Name: </label>
        <input type='text' name='addHelmet' onChange={changeName} /><br />
